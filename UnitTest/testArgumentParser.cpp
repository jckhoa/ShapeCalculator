...
namespace UnitTest
{
	...
	public:
		...
		TEST_METHOD(TestArgumentParser_CircleOption)
		{
			...
			Assert::IsTrue(ap.process(argc1, argv1, errorMessage));
			Assert::IsTrue(ap.getResults().compare("Circle(radius=5) has (perimeter=31.4159,area=78.5398).\n") == 0);
			
			//Invalid shape
			int argc2 = 3;
			char* argv2[3] = { "ShapeCalculator.exe", "-c", "-7.5" };
			Assert::IsTrue(ap.process(argc2, argv2, errorMessage));
			Assert::IsTrue(ap.getResults().compare("Circle(radius=-7.5) is invalid.\n") == 0);

			//Invalid arguments for the option
			int argc4 = 4;
			char* argv4[4] = { "ShapeCalculator.exe", "-c", "5.2", "4." };
			Assert::IsFalse(ap.process(argc4, argv4, errorMessage));
		...
		TEST_METHOD(TestArgumentParser_DonutOption)
		{
			...
			Assert::IsTrue(ap.process(argc1, argv1, errorMessage));
			Assert::IsTrue(ap.getResults().compare("Donut(intern_radius=3,extern_radius=5) has (perimeter=50.2655,area=50.2655).\n") == 0);
			
			//Invalid shape
			int argc2 = 4;
			char* argv2[4] = { "ShapeCalculator.exe", "-d", "-7.5", "2." };
			Assert::IsTrue(ap.process(argc2, argv2, errorMessage));
			Assert::IsTrue(ap.getResults().compare("Donut(intern_radius=-7.5,extern_radius=2) is invalid.\n") == 0);
		...
		TEST_METHOD(TestArgumentParser_SquareOption)
		{
			...
			Assert::IsTrue(ap.process(argc1, argv1, errorMessage));
			Assert::IsTrue(ap.getResults().compare("Square(side=5) has (perimeter=20,area=25).\n") == 0);

			//Invalid shape
			int argc2 = 3;
			char* argv2[3] = { "ShapeCalculator.exe", "-s", "-7.5" };
			Assert::IsTrue(ap.process(argc2, argv2, errorMessage));
			Assert::IsTrue(ap.getResults().compare("Square(side=-7.5) is invalid.\n") == 0);
		...
		TEST_METHOD(TestArgumentParser_RectangleOption)
		{
			...
			Assert::IsTrue(ap.process(argc1, argv1, errorMessage));
			Assert::IsTrue(ap.getResults().compare("Rectangle(width=5,height=3) has (perimeter=16,area=15).\n") == 0);

			//Invalid shape
			int argc2 = 4;
			char* argv2[4] = { "ShapeCalculator.exe", "-r", "-7.5", "2." };
			Assert::IsTrue(ap.process(argc2, argv2, errorMessage));
			Assert::IsTrue(ap.getResults().compare("Rectangle(width=-7.5,height=2) is invalid.\n") == 0);
		...
		TEST_METHOD(TestArgumentParser_RectangularTriangleOption)
		{
			...
			Assert::IsTrue(ap.process(argc1, argv1, errorMessage));
			Assert::IsTrue(ap.getResults().compare("RectangularTriangle(side1=4,side2=3) has (perimeter=12,area=6).\n") == 0);
			
			//Invalid shape
			int argc2 = 4;
			char* argv2[4] = { "ShapeCalculator.exe", "-rt", "-7.5", "2." };
			Assert::IsTrue(ap.process(argc2, argv2, errorMessage));
			Assert::IsTrue(ap.getResults().compare("RectangularTriangle(side1=-7.5,side2=2) is invalid.\n") == 0);
		...
		TEST_METHOD(TestArgumentParser_IsoscelesTriangleOption)
		{
			...
			Assert::IsTrue(ap.process(argc1, argv1, errorMessage));
			Assert::IsTrue(ap.getResults().compare("IsoscelesTriangle(side=5,base=6) has (perimeter=16,area=12).\n") == 0);
			
			//Invalid shape
			int argc2 = 4;
			char* argv2[4] = { "ShapeCalculator.exe", "-it", "5.", "11." };
			Assert::IsTrue(ap.process(argc2, argv2, errorMessage));
			Assert::IsTrue(ap.getResults().compare("IsoscelesTriangle(side=5,base=11) is invalid.\n") == 0);
		...
		TEST_METHOD(TestArgumentParser_TriangleOption)
		{
			...
			Assert::IsTrue(ap.process(argc1, argv1, errorMessage));
			Assert::IsTrue(ap.getResults().compare("Triangle(side1=3,side2=4,side3=5) has (perimeter=12,area=6).\n") == 0);
			
			//Invalid shape
			int argc2 = 5;
			char* argv2[5] = { "ShapeCalculator.exe", "-t", "3.", "6.", "2" };
			Assert::IsTrue(ap.process(argc2, argv2, errorMessage));
			Assert::IsTrue(ap.getResults().compare("Triangle(side1=3,side2=6,side3=2) is invalid.\n") == 0);
		...
		TEST_METHOD(TestArgumentParser_TwoOption)
		{
			...
			Assert::IsTrue(ap.getResults().compare(expected) == 0);
